...
func TestPutHandlerError(t *testing.T) {
	...
	req, err := http.NewRequest("PUT", "/data", strings.NewReader(`{"id": 1, "device_id": "device_id", "device_name": "device_name", "value": 1.0, "type2": "type", "date_time": "2020-01-01T00:00:00Z"}`))
	if err != nil {
		t.Fatal(err)
	}
...
func TestPutHandlerSuccess(t *testing.T) {
	...
	expected := `{"id":1,"student_id":0,"student_name":"","room_id":0}`
	if strings.TrimSpace(rr.Body.String()) != expected {
		t.Errorf("handler returned unexpected body: got %v want %v", rr.Body.String(), expected)
	}
}
